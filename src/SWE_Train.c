...
void SemWE_QsemDerive_Cosine(int word_index, real* derive_distEu)
{
    ...
    for (i = 0; i < kdb_num; ++i)
    {
        ...
        ///
        norm_A = SemWE_VectorNorm(&syn0[index_A*layer1_size]); // |A|
        ...
        dist_AB = SemWE_VectorDot(&syn0[index_A*layer1_size], &syn0[index_B*layer1_size]); //A*B
        dist_CD = SemWE_VectorDot(&syn0[index_C*layer1_size], &syn0[index_D*layer1_size]);
        dist_AB /= (norm_A*norm_B);  // consine distance A*B/(|A||B|)
        ...
        ABCD_Sigmoid   = SemWE_CalcuHinge(CD_minus_AB);     // hinge loss
        derive_Sigmoid = SemWE_DeriveHinge(ABCD_Sigmoid);   // f'
        ...
        if (derive_Sigmoid != 0.0)
        {
            ...
            //D对Wt的梯度=  E [ f' * (Sik对Wi的梯度 - Sij对Wi的梯度 ) ]
